   ...
   while 1:
       ...
       if event2 == '确定':
           ...
           lay3 = [[sg.Menu(menu_def, tearoff=True)],
                   [sg.Text('选择的反应机理为:', font=("times new roman", 15),size=(60, 1),),sg.Button('额外参数')],
           ...
           lay3 += [
               ...
               [sg.Multiline('结果输出', font=("times new roman", 15), size=(27, 8), key='out1'),sg.Multiline('其他系数输出', font=("times new roman", 15), size=(27,8), key='out2'),sg.Multiline('直管爆震室单个工作周期冲量\n平均标准推力', font=("times new roman", 15), size=(27, 8), key='out3')],
       ...
       while 1:
           ...
           if event3 in (None, '退出'):
               ...
               win6_active=True
               lay6=[
                   [sg.Text('已设定的值为：'), sg.Text(('h={}\nA={}\nL={}\nMa={}\nf={}\nc0={}\n'.format(h,A,L,Ma,f,c0)), size=(20, 6), key='input')],
                   [sg.Text('输入额外参数：')],
                   [sg.Text('海拔高度h', font=("times new roman", 15), size=(20, 1)),sg.Input(size=(35, 1), key=('h'))],
                   [sg.Text('爆震室横截面积A', font=("times new roman", 15), size=(20, 1)),sg.Input(size=(35, 1), key=('A'))],
                   [sg.Text('爆震室长度L', font=("times new roman", 15), size=(20, 1)), sg.Input(size=(35, 1),key=('L'))],
                   ...
                   [sg.Text('当量比c0', font=("times new roman", 15), size=(20, 1)),sg.Input(size=(35, 1), key=('c0'))],
                   [sg.Button('确定'),sg.Button('取消'),sg.Button('还原'),sg.Text('',size=(20,1)),sg.Button('帮助',size=(14,1))]
               ]
               win6 = sg.Window('额外参数', default_element_size=(40, 10), grab_anywhere=True).Layout(lay6)
               while 1:
                   event6,values6=win6.read()
                   if event6 in (None, '取消'):
                       win6.close()
                       win6_active=False
                       break
                   elif event6=='还原':
                       ...
                       f=''
                       c0=''
                       sg.Popup('已还原额外参数',font=("times new roman", 15))
                       win6['input'].update(('h={}\nA={}\nL={}\nMa={}\nf={}\nc0={}\n'.format(h,A,L,Ma,f,c0)))
                   elif event6=='确定':
                       pd6=1
                       for i in ['h','A','L','Ma','f','c0']:
                           try:
                               values6[i]=float(values6[i])
                           except:
                               sg.Popup('请输入正确的反应组分和初始条件！')
                               pd6=0
                               break
                       if pd6==1:
                           sg.Popup('已设定额外参数', font=("times new roman", 15))
                           h = float(values6['h'])
                           A = float(values6['A'])
                           L = float(values6['L'])
                           Ma = float(values6['Ma'])
               ...
               try:
                   ...
                   if pd6 == 1 :
                        ...
                        #dictm字典中储存每个反应物的质量
                        dictm={'H2':2,'CH3CHO':44,'CH4':16}
                        #dictn字典储存每个反应物完全反应所需的氧气的物质的量
                        dictn={'H2':0.5,'CH3CHO':2.5,'CH4':2}
